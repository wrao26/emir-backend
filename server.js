...
async function scrapeProduct(url) {
  ...
  try {
    ...
    // Tenta esperar por elemento de preço (até 8 segundos)
    await page.waitForSelector('[class*="price"], [itemprop="price"], .price, #price', { timeout: 8000 }).catch(() => {
      console.log('[SCRAPE] Elemento de preço não encontrado via waitForSelector');
    });
    ...
    // Log do HTML para debug — encontra onde está o preço
    const htmlSnippet = await page.evaluate(() => {
      // Procura qualquer elemento com preço visível
      const allEls = document.querySelectorAll('*');
      const priceEls = [];
      for (const el of allEls) {
        const txt = el.textContent?.trim() || '';
        // Procura padrão de preço (ex: 129,90 ou 129.90 ou €129)
        if (/^[€£$]?\s*\d{1,4}[.,]\d{2}$/.test(txt) && el.children.length === 0) {
          priceEls.push({
            tag: el.tagName,
    ...
    console.log('[SCRAPE] Elementos com preço encontrados:', JSON.stringify(htmlSnippet));
    ...
    const dados = await page.evaluate(() => {
      ...
      // Preço via seletores CSS — cobre Amazon, Zara, Massimo Dutti, Nike, ASOS, Farfetch, Shein, H&M
      if (!preco) {
        const sels = [
          // Amazon
          ...
          '.price-current .price-current__amount',
          ...
          '.f-price', '.userPrice',
          // Genérico
          '[class*="price"]:not([class*="original"]):not([class*="was"]):not([class*="old"]):not([class*="cross"])',
          '[class*="Price"]:not([class*="Original"]):not([class*="Was"]):not([class*="Cross"])',
