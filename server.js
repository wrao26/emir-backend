// ══════════════════════════════════════════════════
// EMIR.COM — Backend v5 — Solução definitiva scraping
// ══════════════════════════════════════════════════
...
async function notificarEmail(enc) {
  ...
  const html = `<div style="font-family:Arial,sans-serif;max-width:600px;margin:auto">
    ...
    <div style="background:#f8f9fa;padding:24px;border-radius:0 0 8px 8px">
      ...
      <div style="margin-top:20px"><a href="${enc.produto_url}" style="background:#1A6BFF;color:#fff;padding:12px 24px;border-radius:6px;text-decoration:none;font-weight:700">Ver Produto →</a></div>
    </div></div>`;
  try {
    await axios.post('https://api.resend.com/emails', {
      from: 'Emir.com <noreply@emir.com>', to,
...
// ════════════════════════════════════════════════
// PUPPETEER — SCRAPER DEFINITIVO
// 4 estratégias em cascata:
// 1. Interceptar respostas de API da página
// 2. JSON-LD estruturado
// 3. Meta tags Open Graph
// 4. Seletores CSS por loja + genéricos
// ════════════════════════════════════════════════
...
// Extrai número de preço de qualquer texto
function extractPrice(txt) {
  if (!txt) return null;
  // Remove símbolos de moeda e espaços
  const clean = txt.replace(/[€£$\s]/g, '').trim();
  // Tenta formatos: 199.95 | 199,95 | 1.199,95 | 1,199.95
  const patterns = [
    /^(\d{1,4}[.,]\d{2})$/,                    // 199.95 ou 199,95
    /^(\d{1,3}[.,]\d{3}[.,]\d{2})$/,           // 1.199,95 ou 1,199.95
    /(\d{1,4}[.,]\d{2})/,                       // qualquer parte
  ];
  for (const p of patterns) {
    ...
    if (m) {
      ...
      // Se ficou com dois pontos (ex: 1199.95 estava certo)
      if ((v.match(/\./g) || []).length > 1) v = v.replace('.', '');
      const f = parseFloat(v);
      if (f > 0 && f < 100000) return f;
    }
  }
  return null;
}
...
async function scrapeProduct(url) {
  ...
  try {
    ...
    // ESTRATÉGIA 1: Interceptar respostas de API da página
    let apiPrice = null;
    await page.setRequestInterception(true);
    page.on('request', req => {
      const rt = req.resourceType();
      // Bloqueia apenas media — mantém scripts e XHR para o preço carregar
    ...
    page.on('response', async resp => {
      ...
      try {
        ...
        // Procura campos de preço em JSON
        const priceFields = [
          'currentPrice','salePrice','finalPrice','price','value',
          'amount','discountedPrice','offerPrice','sellingPrice',
          'priceWithTax','priceFormatted'
        ];
        try {
          const json = JSON.parse(text);
          const findPrice = (obj, depth=0) => {
            if (depth > 6 || apiPrice) return;
            if (typeof obj === 'number' && obj > 0 && obj < 100000) {
              // Verifica se o campo pai tem nome de preço
              return;
            }
            if (typeof obj === 'object' && obj !== null) {
              for (const [k, v] of Object.entries(obj)) {
                if (apiPrice) return;
                if (priceFields.some(f => k.toLowerCase().includes(f.toLowerCase()))) {
                  const p = typeof v === 'number' ? v : extractPrice(String(v));
                  if (p && p > 0 && p < 100000) { apiPrice = p; return; }
                }
                if (typeof v === 'object') findPrice(v, depth+1);
              }
            }
          };
          findPrice(json);
          if (apiPrice) console.log('[SCRAPE] Preço via API JSON:', apiPrice, 'de', u.slice(0,80));
        } catch {}
      } catch {}
    });
    ...
    await page.goto(url, { waitUntil: 'load', timeout: 45000 }).catch(async () => {
      console.log('[SCRAPE] Timeout parcial, a continuar com o que carregou...');
    });

    // Espera para JS e APIs carregarem
    ...
    console.log('[SCRAPE] Título:', pageTitle);

    // ESTRATÉGIAS 2, 3, 4 — dentro do browser
    const dados = await page.evaluate(() => {
        ...
        try {
          ...
          const findInObj = (obj, depth=0) => {
            ...
            if (price !== undefined && price !== null) {
              ...
              if (v > 0 && v < 100000) { preco = v; if (curr) moeda = curr.toUpperCase(); return; }
            }
            for (const val of Object.values(obj)) {
              if (typeof val === 'object') findInObj(val, depth+1);
            }
          };
          objs.forEach(o => findInObj(o));
        } catch {}
      });
      ...
      // ESTRATÉGIA 4: Seletores CSS por loja + genéricos
      if (!preco) {
        const sels = [
          // Inditex (Zara, Massimo Dutti, Pull&Bear, Bershka, Stradivarius)
          '.price-current__amount', '[class*="price-current__amount"]',
          '[class*="productPrice__amount"]', '[class*="product-price__amount"]',
          ...
          // Farfetch
          '[data-testid="price-label"]', '[data-component="PriceWithDiscounts"]',
          // Shein
          '.product-intro__head-mainprice', '.S-product-item__price',
          // H&M
          '.product-item-price .price', '.hm-price',
          // Fnac
          '.f-price .amount', '.userPrice .price',
          // El Corte Inglés
          '.precio-producto', '.precio',
          // IKEA
          '.range-revamp-price__integer',
          // Booking / viagens (ignorar)
          // Genérico — funciona na maioria das lojas
          '[itemprop="price"]',
          '[class*="current-price"]:not([class*="was"])',
          '[class*="currentPrice"]:not([class*="Was"])',
      ...
      return { titulo, imagem, preco, moeda };
    }).catch(e => {
      console.log('[SCRAPE] Erro no evaluate:', e.message);
      return { titulo: pageTitle, imagem: null, preco: null, moeda: 'EUR' };
    });

    result.titulo = dados.titulo || pageTitle || null;
    result.imagem = dados.imagem || null;
    result.preco  = dados.preco || apiPrice || null;
    ...
    console.log('[SCRAPE] Resultado final:', JSON.stringify(result));

  } catch(e) {
    console.error('[SCRAPE] ERRO:', e.message);
  } finally {
...
app.get('/', (_, res) => res.json({ status: 'ok', app: 'Emir Backend v5' }));

app.get('/api/cambio', async (_, res) => {
  res.json({ eur_aoa: await getRate() });
});
...
app.post('/api/clientes/login', loginLimiter, async (req, res) => {
  ...
  if (!cliente.ativo) return res.status(403).json({ erro: 'Conta suspensa.' });
...
app.post('/api/clientes/favoritos', authCliente, async (req, res) => {
  ...
  if (error) {
    if (error.code === '23505') return res.status(409).json({ erro: 'Já está nos favoritos.' });
...
// ════════════════════════════════════════════════
// ENCOMENDAS — só clientes com conta
// ════════════════════════════════════════════════
...
app.post('/api/encomendas', authCliente, async (req, res) => {
  ...
  res.status(201).json({
    ...
    mensagem: `Encomenda ${data.numero} registada! Receberás confirmação por email.`
  });
});
...
app.get('/api/admin/dashboard', authAdmin, async (_, res) => {
  ...
  res.json({
    total: total.count||0, pendentes: pendentes.count||0, em_curso: pagas.count||0,
    entregues: entregues.count||0, clientes: clientes.count||0,
    receita_kz: Math.round(receita), recentes: recentes||[]
  });
});
...
app.get('/api/admin/relatorios/financeiro', authAdmin, async (req, res) => {
  ...
  const por_metodo={}, por_provincia={}, por_dia={};
  lista.forEach(e=>{
    const m=e.metodo_pag||'Outro'; if(!por_metodo[m]) por_metodo[m]={count:0,total:0}; por_metodo[m].count++; por_metodo[m].total+=e.total_kz||0;
    const p=e.entrega_prov||'?'; if(!por_provincia[p]) por_provincia[p]={count:0,total:0}; por_provincia[p].count++; por_provincia[p].total+=e.total_kz||0;
    const d=e.criado_em.slice(0,10); if(!por_dia[d]) por_dia[d]={encomendas:0,receita:0}; por_dia[d].encomendas++;
    if(e.estado==='Entregue') por_dia[d].receita+=e.total_kz||0;
  });
  res.json({
    periodo:{inicio:dataInicio.toISOString(),fim:dataFim.toISOString()},
    resumo:{total_encomendas:lista.length,entregues:entregues.length,em_curso:lista.filter(e=>!['Entregue','Cancelado','Pendente'].includes(e.estado)).length,pendentes:lista.filter(e=>e.estado==='Pendente').length,canceladas:canceladas.length,taxa_conversao:lista.length?Math.round((entregues.length/lista.length)*100):0},
    financeiro:{receita_total_kz:Math.round(receita),taxas_emir_kz:Math.round(taxas),valor_envio_kz:Math.round(entregues.reduce((s,e)=>s+(e.envio_kz||0),0)),ticket_medio_kz:entregues.length?Math.round(receita/entregues.length):0},
    por_metodo,
    por_provincia:Object.entries(por_provincia).sort(([,a],[,b])=>b.count-a.count).slice(0,10).map(([provincia,v])=>({provincia,...v})),
...
app.patch('/api/admin/clientes/:id/ativo', authAdmin, async (req, res) => {
  ...
  const {data,error}=await supabase.from('clientes').update({ativo}).eq('id',req.params.id).select('id,nome,ativo').single();
  if(error) return res.status(500).json({erro:error.message});
  res.json(data);
});
...
app.post('/api/admin/produtos', authAdmin, async (req,res) => {
  ...
  const {data,error}=await supabase.from('produtos').insert({nome,categoria:categoria||'Outros',loja,preco_eur,imagem_url,descricao}).select().single();
  if(error) return res.status(500).json({erro:error.message});
  res.status(201).json(data);
});
...
app.post('/api/admin/password', authAdmin, async (req,res) => {
  ...
  if(!password_nova||password_nova.length<8) return res.status(400).json({erro:'Password mínimo 8 caracteres.'});
  const {data:admin}=await supabase.from('admins').select('password_hash').eq('id',req.admin.id).single();
  if(!await bcrypt.compare(password_atual,admin.password_hash)) return res.status(401).json({erro:'Password actual incorrecta.'});
